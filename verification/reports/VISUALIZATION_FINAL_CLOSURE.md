...
3. Replaced the filled Mesh alias with a deterministic VDW-sample wire/lattice projection using bounded renderer edges/nodes.
...
6. Kept Cartoon, Ribbon, Trace, and Putty on distinct renderer profiles; Putty maps canonical per-residue B-factor values to variable cartoon thickness and does not fabricate missing values.
...
| Putty | SUPPORTED_WITH_LIMITATIONS | Canonical per-residue B-factor | 3Dmol cartoon trace with grouped variable thickness | Varied-B-factor fixture and screenshot | Missing B-factors remain unavailable | `canonical-polymer-putty-bfactor.v1` |
...
| Mesh | SUPPORTED_WITH_LIMITATIONS | Canonical coordinates + VDW sample profile | Application-native exposed-point wire/lattice | SES → Mesh screenshot | Not a quantitative molecular mesh | `surface-mesh.v2` |
| Dots | SUPPORTED_WITH_LIMITATIONS | Canonical coordinates + VDW sample profile | Bounded application-native sampled GLShape points | 4DJW Dots screenshot; ready/count diagnostics | Display sample only, not SASA | `surface-dots.v1` |
...
- `npm test`: PASS — web 14 files / 53 tests; API 2 files / 9 tests.
...
- `npm run test:e2e`: PASS — 51 tests, including all existing accepted UI tests and 5 V-FINAL tests.
...
- VDW, SAS, SES, Mesh lattice, Dots, and Dot Surface profile transitions;
...
`empty-workstation.png`, `uploaded-protein-cartoon-ligand-sticks.png`, `rcsb-4djw-cartoon-ligand-sticks.png`, `polymer-cartoon.png`, `polymer-ribbon.png`, `polymer-trace.png`, `polymer-putty.png`, `space-filling-ligand-only.png`, `nonbonded-crosses.png`, `nonbonded-spheres.png`, `surface-vdw.png`, `surface-sas.png`, `surface-ses.png`, `surface-mesh-lattice.png`, `surface-dots.png`, `surface-dot-surface.png`, `surface-transparent.png`, `selected-pick.png`, `cleared-pick.png`, `chain-labels.png`, `analysis-h-bonds.png`, `analysis-contacts.png`, `analysis-clash.png`, and `analysis-pocket-unavailable.png`.
...
- Mesh is a deterministic exposed-point lattice. Dots and Dot Surface are bounded presentation sampling and must not be read as quantitative area values.
...
Source, tests, docs, and evidence were committed on `fix/visualization-final-closure` and pushed to `new-origin`. The final remote branch was verified with `git ls-remote` after push.
