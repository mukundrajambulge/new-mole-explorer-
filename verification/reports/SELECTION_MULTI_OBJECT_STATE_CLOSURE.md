...
- **SELECTION IMPLEMENTATION STATUS:** COMPLETE FOR CURRENT SUPPORTED PROFILE
- **FULL PYMOL SELECTION CONFORMANCE STATUS:** BLOCKED ONLY ON BYFRAGMENT EXTERNAL SCIENTIFIC DEPENDENCY
- **BYFRAGMENT STATUS:** MISSING_DEPENDENCY / ORACLE_PENDING
- **PRODUCT BEHAVIOR:** FAIL-CLOSED; no fabricated fragment assignment is produced when the canonical dependency is absent.

The supported selection implementation, live regressions, multi-object workspace, multi-state rendering, and visualization regression suite pass in the authoritative repository and are now sealed. Cross-object spatial queries require and record an explicit coordinate-frame policy; every coordinate-dependent selection also records the exact per-object coordinate state scope consulted, including cached and bound plans. The pinned PyMOL source has executed against the shared fixture for 85 direct forms: 79 pass and 6 return native errors; the comparison ledger promotes only exact forms or documented aliases. The application binds RenderProjection representation, color, label, and representation-specific color selectors, evaluates canonical fragment assignments and bounded ring topology without renderer-derived shortcuts, implements a versioned VDW surface-gap profile with strict missing-radius handling, binds `pepseq` to a revision-stamped canonical one-letter peptide sequence dataset, preserves source-backed crystallographic unit-cell parameters for bounded `bycell` membership, retains the actual official remote retrieval provider and URI when RCSB falls back to the wwPDB partner endpoint, promotes complete source-declared mmCIF partial charges without inference, and produces a bounded, revision-matched canonical chemistry-role dataset for `donors`/`acceptors` when the pinned PyMOL compatibility inputs are sufficient. `byfragment` is the single external scientific dependency carve-out; it is not fabricated when its source assignments are absent. Dynamic arbitrary-property evaluation is explicitly unavailable and is covered by the pinned native rejection of `foo = bar`.
...
- `donors` and `acceptors` consume only a complete `canonical-chemistry-roles-v1` dataset whose molecular revision matches the canonical structure. The backend produces that dataset with the bounded pinned PyMOL compatibility profile from canonical atoms, coordinates, formal charge, and bond topology; the evaluator records the scientific profile and fails closed when those inputs are insufficient. No renderer-derived heuristic is promoted to scientific state, and broader protonation/tautomer/resonance perception is not claimed. The pinned chemistry-perception audit is recorded in [pymol-chemistry-role-source.md](../selection/pymol-chemistry-role-source.md).
...
- Rows: **87**
- Implementation: **85 VERIFIED_WORKING**, **1 MISSING_DEPENDENCY**, **1 INTENTIONALLY_UNSUPPORTED**
- Status-category summary: **0 BROKEN**, **0 MISSING_IMPLEMENTABLE**, **1 MISSING_DEPENDENCY**, **0 RESEARCH_REQUIRED**, **1 ORACLE_PENDING**, **1 INTENTIONALLY_UNSUPPORTED**
- Live-browser status: **92 pass/accepted diagnostic outcomes** across the expanded matrix exercise
- Live evidence: [selection-live-evidence.json](../selection/selection-live-evidence.json) records all **92** attempts, with **0 browser-console errors, 0 page errors, 0 network failures, 0 atom-count mismatches, 0 viewer/panel membership mismatches**, and subsequent targeting checks retaining the active selection hash.
- `in`, `bycalpha`, `bymolecule`, `byfragment`, `byring`, and `bycell` now use canonical tuple, residue-CA, connected-component, canonical-fragment-assignment, bounded-cycle, and source-backed fractional-cell-membership semantics respectively. `donors` and `acceptors` now use the backend-produced, revision-bound bounded PyMOL chemistry-role profile and are live-verified at 7 and 4 atoms on the mini fixture. `visible` is now bound to an explicit presentation context derived from render directives; `rep`, `color`, `label`, `cartoon_color`, and `ribbon_color` are likewise evaluated from the current RenderProjection and never from 3Dmol internals. Generic atom color and representation-scoped color are separate namespaces; `set cartoon_color|ribbon_color, <color>, <query>` is the explicit representation-scoped mutation path. These selectors never alias scientific `all`. `like`, the application implicit-adjacency profile, and the new spatial/topology forms are live-verified; malformed shorthand and missing scientific dependencies remain truthful diagnostics.
...
- Canonical chemistry roles: `donors` and `acceptors` **PASS** on the mini fixture through the backend-produced bounded pinned-PyMOL profile (7 and 4 atoms respectively); inputs without sufficient canonical topology remain **MISSING_DEPENDENCY** and no broader chemistry perception is claimed.
...
- Latest chemistry-role producer: `apps/api/src/structures/chemistryRoles.ts`, `apps/api/src/structures/ingestion.ts`, and `apps/api/src/structures/ingestion.test.ts`; the bounded pinned profile produces 7 donor and 4 acceptor fixture assignments, while insufficient canonical topology remains fail-closed
- Latest single-object workspace dataset regression: `apps/web/src/workspace/workspaceModel.ts` and `apps/web/src/workspace/workspaceModel.test.ts`; canonical chemistry IDs remain unscoped for a single object and are namespaced only for multi-object universes
...
- `npm run lint` — **PASS**
- `npm test` — **PASS: web 18 files / 94 tests; API 2 files / 22 tests**
- `npm run test --workspace @molecular/web -- --run src/selection/selectionEngine.test.ts src/workspace/workspaceModel.test.ts` — **PASS: 2 files / 31 tests; missing, stale, incomplete, single-object, and multi-object chemistry/fragment datasets are covered**
...
- `npm run test --workspace @molecular/web -- --run src/selection/selectionEngine.test.ts src/workspace/workspaceModel.test.ts` — **PASS: 2 files / 31 tests; chemistry-role and fragment provenance/coverage gates are covered**
- `npm run verify:selection-matrix` — **PASS: 87 rows; JSON regenerated**
- `npm run test:e2e -- --workers=1` — **PASS: 82 / 82; 13.7 minutes**
- `npx playwright test tests/e2e/selection-matrix-live.spec.ts` — **PASS: 1 / 1; 92 live queries, including donors=7 and acceptors=4**
- Fresh live rerun after the single-object dataset namespace fix — **PASS: 1 / 1; 92 live queries; 3.2 minutes; exit 0**
...
15. Import `tests/fixtures/mini-protein.pdb`, run `donors` and `acceptors`, and confirm 7 and 4 atoms respectively; inspect the API load result to confirm the revision-matched chemistry-role provenance cites the pinned profile.
...
- Crystallographic `pbc`/`symmetry` and broader chemistry remain explicit unsupported or missing-dependency gates. `bycell` now has a bounded source-backed fractional-cell-membership profile, but it does not expand symmetry mates or periodic images. Donor/acceptor selection now has a backend-produced, revision-bound canonical chemistry-role dataset for inputs sufficient for the bounded pinned-PyMOL profile; inputs without sufficient topology or supported chemistry inputs still fail closed without heuristic roles. Unknown properties and unknown VDW radii fail closed. `gap` is implemented only for the declared versioned element-radius profile; `byring` is implemented only for the declared bounded-cycle profile, while `byfragment` requires canonical source-backed fragment assignments. These profiles are not claimed as full PyMOL chemistry perception.
...
- `donors` / `acceptors`: the bounded pinned-PyMOL compatibility producer is now implemented and live-verified on the mini fixture; the remaining research boundary is broader chemistry perception covering protonation, tautomer, resonance, complete chemical-component graphs, and unknown components. The pinned source-level inputs and decision are recorded in [pymol-chemistry-role-source.md](../selection/pymol-chemistry-role-source.md). Required next research fixture set: benzene, pyridine, pyrrole, amide, carboxylate, protonated amine, and unknown-component cases; suggested stage: R03-04 chemistry-perception addendum plus Research-02 chemical-graph/CCD integration.
...
- `byfragment`: the exact question is whether the accepted profile should expose PyMOL editor fragment assignments directly or admit a different chemistry-defined fragment dataset. The pinned source uses editor fragment assignments, while current PDB/mmCIF ingestion supplies neither those assignments nor an equivalent canonical dataset; `_entity.pdbx_fragment` is only entity-level descriptive text and cannot provide complete atom membership. Connected components are reserved for `bymolecule`. The source-level audit is recorded in [pymol-byfragment-source.md](../selection/pymol-byfragment-source.md). Required next research fixture: disconnected editor fragments and a source-backed fragment-assignment payload; suggested stage: R-PYMOL chemistry/fragment addendum.
...
## Future external-dependency handoff: `byfragment`

This handoff is intentionally separate from the sealed supported Selection implementation. It is the only open external scientific dependency and is not an implementation-search loop.

- **Required scientific input:** an accepted definition of fragment identity and membership for the selected compatibility profile, plus a source-backed atom-to-fragment assignment for every canonical atom in a positive fixture containing at least two disconnected fragments. The decision must state whether the profile exposes PyMOL editor-fragment assignments directly or admits a chemistry-defined alternative.
- **Exact dataset contract:** `CanonicalFragmentDataset` in `packages/contracts/src/index.ts`, with `profileVersion: "canonical-fragment-assignment-v1"`; non-empty `datasetId`, `assignmentSource`, and `provenance`; `molecularRevision` exactly equal to the canonical structure revision; and `atomFragmentMap` covering every canonical `StableAtomID` exactly once with non-empty fragment identifiers and no unknown keys. Multi-object views must remap the map by durable `ObjectID` while preserving fragment identity within each object scope.
- **Why current sources are insufficient:** the pinned PyMOL operator reads internal editor-fragment selections, while ordinary admitted PDB/mmCIF ingestion supplies no equivalent complete per-atom assignment. `_entity.pdbx_fragment` is descriptive entity text, not atom membership, and the audited 4DJW mmCIF contains no fragment field. Bond connected components are the separate `bymolecule` authority.
- **Evidence required to unblock:** a research-approved profile/addendum; a reproducible source-backed fixture with disconnected fragments and expected seed-to-fragment expansion; direct pinned-PyMOL oracle output for that fixture; backend ingestion or an explicitly admitted assignment producer that validates the dataset contract; unit, multi-object, and live-console verification of positive, empty, stale, and incomplete cases; and updated matrix/oracle evidence with no renderer-derived identity.
- **Prohibited fallbacks:** `bymolecule`, connected/bond components, residue or chain grouping, distance grouping, renderer groups, coordinate inference, fragment-library filenames, per-atom hints, or arbitrary chemistry heuristics.
- **Recommended future task:** `R-PYMOL chemistry/fragment addendum` with the disconnected-editor-fragment fixture and canonical assignment producer decision.
