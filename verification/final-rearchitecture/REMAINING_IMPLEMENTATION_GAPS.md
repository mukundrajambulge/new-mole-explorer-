...
| --- | --- | --- |
| Biological formats | Coordinate objects remain PDB, mmCIF/CIF, PQR, SDF/MOL, bounded single-frame XYZ, MOL2, and PDBQT. FASTA/FASTQ/GenBank/EMBL, DX/MRC/CCP4, multi-frame XYZ, GRO, PSF, PRMTOP, and SMILES now route to typed dedicated viewers without coordinate fabrication. DCD, TRR, and XTC decode bounded coordinate frames. Atom-count-checked PSF/PRMTOP pairing enriches trajectory metadata without fabricating coordinates. | Add larger corpus and performance gates before making broader research-trajectory claims. |
