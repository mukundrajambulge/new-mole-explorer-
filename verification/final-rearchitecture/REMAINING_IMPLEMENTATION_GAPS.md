...
| --- | --- | --- |
| Biological formats | Coordinate objects remain PDB, mmCIF/CIF, PQR, SDF/MOL, bounded single-frame XYZ, MOL2, and PDBQT. FASTA/FASTQ/GenBank/EMBL, DX/MRC/CCP4, multi-frame XYZ, GRO, PSF, PRMTOP, and SMILES now route to typed dedicated viewers without coordinate fabrication. DCD and TRR decode bounded coordinate frames; XTC remains an explicit metadata-only entry. | Add larger corpus and performance gates; implement full XTC frame decoding and topology-coordinate pairing before making research trajectory claims. |
