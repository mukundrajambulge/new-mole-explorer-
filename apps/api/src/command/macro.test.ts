...
describe("R10 bounded macro/batch execution", () => {
  it("rejects cycles and executes a deterministic DAG", () => {
    ...
    const cyclic = { schemaVersion: 1 as const, macroId: "cycle", name: "cycle", version: "1", nodes: [{ nodeId: "a", command, dependsOn: ["b"] }, { nodeId: "b", command, dependsOn: ["a"] }], maxNodes: 4, maxIterations: 4, errorPolicy: "STOP_ON_ERROR" as const, deterministic: true, contentHash: "macro-hash", provenance: { createdAt: new Date().toISOString(), sourceHash: "x" } };
