...
describe("R10 safe command parser security corpus", () => {
  const corpus = [
    ...
    "python print(1)", "javascript:alert(1)", "exec(\"x\")", "run echo bad", "system rm -rf /", "select all && echo bad",
    "select all; python print(1)", "select all; get_view", "select ${process.env.SECRET}", "color red, all > out",
    "select (((((((((((((((((((((((((((((((all)))))))))))))))))))))))))))))))",
  ];
  ...
  it("rejects host-code and process escape attempts before dispatch", () => {
    for (const source of ["python print(1)", "javascript:alert(1)", "exec(\"x\")", "run echo bad", "system rm -rf /", "select all && echo bad", "select all; python print(1)", "select ${process.env.SECRET}"]) {
