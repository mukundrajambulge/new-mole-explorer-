...
const executionIdFor = (command: CanonicalCommand, attempt = 1): string => `exec:${sha256(`${command.commandId}:${command.semanticHash}:attempt:${attempt}`).slice(0, 24)}${attempt > 1 ? `:attempt-${attempt}` : ""}`;
...
export class CommandDispatcher {
  ...
  private readonly jobCommands = new Map<string, CanonicalCommand>();
  ...
  constructor(options: { dataRoot?: string; settings?: SettingStore; history?: ActionRecordStore; bindings?: CommandBindingContext; revisions?: RevisionProvider } = {}) {
    ...
    this.register("view.get", () => json({ projection: "PERSPECTIVE", fieldOfView: 20, center: [0, 0, 0], distance: 0, rotation: [0, 0, 0, 1] }));
    this.register("setting.set", ({ command }) => {
      const name = String(command.normalizedArgs.name ?? "");
      const scope = String(command.normalizedArgs.scope ?? "GLOBAL") as Parameters<SettingStore["set"]>[2];
    ...
    this.register("setting.get", ({ command }) => {
      const result = this.settings.getResult(String(command.normalizedArgs.name ?? ""), String(command.normalizedArgs.scope ?? "GLOBAL") as Parameters<SettingStore["getResult"]>[1], typeof command.normalizedArgs.targetId === "string" ? command.normalizedArgs.targetId : undefined);
      return json(result.value ?? { diagnostic: result.diagnostic ?? { code: "INVALID_SETTING", message: "Setting was not found." } });
    });
    this.register("setting.unset", ({ command }) => json(this.settings.unset(String(command.normalizedArgs.name ?? ""), String(command.normalizedArgs.scope ?? "GLOBAL") as Parameters<SettingStore["unset"]>[1], typeof command.normalizedArgs.targetId === "string" ? command.normalizedArgs.targetId : undefined)));
    this.register("registry.unimplemented", () => json({ accepted: false, capability: "UNSUPPORTED_CAPABILITY" }));
  }
  ...
  private makeRecord(command: CanonicalCommand, executionId: string, status: ActionRecord["status"], diagnostics: readonly CommandDiagnostic[], startedAt?: string, completedAt?: string, attempt = 1): ActionRecord {
    return { schemaVersion: 1, actionRecordId: `action:${executionId.slice(5)}`, commandId: command.commandId, executionId, rawIntentRef: command.origin.sourceTextRef ?? `intent:${command.origin.sourceHash}`, sourceHash: command.origin.sourceHash, canonicalCommand: { ...command, origin: { ...command.origin, ...(command.origin.sourceText ? { sourceText: redactSource(command.origin.sourceText) } : {}) } }, registryVersion: COMMAND_REGISTRY_VERSION, commandSchemaVersion: COMMAND_SCHEMA_VERSION, compatibilityProfile: command.origin.profile, policyVersion: "r10-safe-policy.v1", handlerVersion: "r10-handler-boundary.v1", inputRevisionRefs: Object.keys(command.expectedRevisions ?? {}), inputArtifactHashes: [], outputRefs: [], outputHashes: [], validation: diagnostics, attempt, inputRef: `input:${command.origin.sourceHash}`, status, submittedAt: command.submittedAt, ...(startedAt ? { startedAt } : {}), ...(completedAt ? { completedAt } : {}), ...(command.parentCommandId ? { parentCommandId: command.parentCommandId } : {}), correlationId: command.correlationId, ...(command.idempotencyKey ? { idempotencyKey: command.idempotencyKey } : {}), diagnostics, redactedSecrets: command.origin.sourceText && redactSource(command.origin.sourceText) !== command.origin.sourceText ? ["sourceText"] : [] };
  }
  ...
  private canonicalDiagnostics(command: CanonicalCommand): readonly CommandDiagnostic[] {
    ...
    if (command.commandVersion !== resolution.spec.registryVersion || command.schemaVersion !== COMMAND_SCHEMA_VERSION) return [commandDiagnostic("UNSUPPORTED_COMMAND_SEMANTICS", `Command registry/schema version ${command.commandVersion}/${command.schemaVersion} is not active; expected ${resolution.spec.registryVersion}/${COMMAND_SCHEMA_VERSION}.`)];
  ...
  private run(command: CanonicalCommand, attempt = 1): CommandResult {
    const executionId = executionIdFor(command, attempt);
    const startedAt = now();
    const security = [...this.canonicalDiagnostics(command), ...this.preconditionDiagnostics(command), ...this.securityDiagnostics(command)];
    if (security.length) {
      const record = this.makeRecord(command, executionId, "FAILED", security, startedAt, now(), attempt);
    ...
    if (!("spec" in resolution)) {
      ...
      const record = this.makeRecord(command, executionId, "FAILED", diagnostics, startedAt, now(), attempt);
      this.history.put(record);
    ...
    const handler = this.handlers.get(resolution.spec.handlerKey);
    if (!handler) {
      const diagnostics = [commandDiagnostic("UNSUPPORTED_CAPABILITY", `${resolution.spec.canonicalName} has no admitted executable handler in the current profile.`)];
      const record = this.makeRecord(command, executionId, "FAILED", diagnostics, startedAt, now(), attempt);
      this.history.put(record);
      return { commandId: command.commandId, executionId, status: "FAILED", diagnostics, warnings: [], actionRecordId: record.actionRecordId, provenanceRef: record.actionRecordId };
    }
    try {
      ...
      const record = this.makeRecord(command, executionId, "SUCCEEDED", diagnostics, startedAt, now(), attempt);
      this.history.put(record);
  ...
  dispatch(request: DispatchRequest): CommandResult {
    ...
    if (idempotencyKey) {
      ...
      if (pendingJobId) {
        ...
        if (pending) return { commandId: pending.commandId, executionId: pending.executionId, status: "SUCCEEDED", diagnostics: [], warnings: [], job: { jobId: pending.jobId, state: pending.state } };
  ...
  private enqueue(command: CanonicalCommand, attempt = 1, previousAttemptId?: string): CommandResult {
    const executionId = executionIdFor(command, attempt);
    const jobId = `job:${executionId.slice(5)}`;
    const createdAt = now();
    const job: CommandJob = { jobId, commandId: command.commandId, executionId, attempt, state: "Created", createdAt, updatedAt: createdAt, ...(previousAttemptId ? { previousAttemptId } : {}) };
    this.jobs.set(jobId, job);
    this.jobCommands.set(jobId, command);
    if (command.idempotencyKey) this.pendingIdempotency.set(command.idempotencyKey, jobId);
    this.jobs.set(jobId, { ...job, state: "Queued", updatedAt: now() });
    queueMicrotask(() => {
      const current = this.jobs.get(jobId);
      if (!current || current.state === "Cancelled") return;
      this.jobs.set(jobId, { ...current, state: "Running", updatedAt: now() });
      const result = this.run(command, attempt);
      this.jobs.set(jobId, { ...this.jobs.get(jobId)!, state: result.status === "SUCCEEDED" ? "Completed" : result.status === "CANCELLED" ? "Cancelled" : "Failed", updatedAt: now(), result });
    });
  ...
  cancel(jobId: string): CommandJob | undefined {
    ...
    const next = { ...job, state: "Cancelled" as CommandJobState, cancellationRequestedAt: now(), updatedAt: now() };
    this.jobs.set(jobId, next);
    return next;
  }

  retry(jobId: string): CommandJob | undefined {
    const previous = this.jobs.get(jobId);
    const command = this.jobCommands.get(jobId);
    if (!previous || !command || previous.state !== "Failed") return undefined;
    const retryResult = this.enqueue({ ...command, idempotencyKey: undefined, submittedAt: now() }, previous.attempt + 1, previous.executionId);
    return retryResult.job ? this.jobs.get(retryResult.job.jobId) : undefined;
  }

  replay(actionRecordId: string, mode: CommandReplayMode = "COMMAND_REPLAY"): CommandResult {
    ...
    if (mode === "REVIEW") return { commandId: record.commandId, executionId: `review:${record.executionId.slice(5)}`, status: "SUCCEEDED", payload: json({ mode, canonicalCommand: record.canonicalCommand, diagnostics: record.diagnostics, attempt: record.attempt }), diagnostics: [], warnings: [], actionRecordId: record.actionRecordId, provenanceRef: record.actionRecordId };
    ...
    const command = { ...record.canonicalCommand, requestedMode: "SYNC" as const, idempotencyKey: undefined, origin: { ...record.canonicalCommand.origin, surface: "REST" as const }, ...(mode === "REPRODUCTION_ATTEMPT" ? { correlationId: `reproduction:${record.executionId}` } : {}) };
    return this.run(command);
  }
