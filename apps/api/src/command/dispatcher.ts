...
import { compileSafeCommand, sha256 } from "./compiler.js";
...
export class CommandDispatcher {
  ...
  constructor(options: { dataRoot?: string; settings?: SettingStore; history?: ActionRecordStore } = {}) {
    ...
    this.register("setting.get", ({ command }) => json(this.settings.get(String(command.normalizedArgs.name ?? ""), String(command.normalizedArgs.scope ?? "session") as Parameters<SettingStore["get"]>[1], typeof command.normalizedArgs.targetId === "string" ? command.normalizedArgs.targetId : undefined) ?? { diagnostic: { code: "INVALID_SETTING", message: "Setting was not found." } }));
  ...
  private run(command: CanonicalCommand): CommandResult {
    ...
    const startedAt = now();
    const security = this.securityDiagnostics(command);
    if (security.length) {
  ...
  dispatch(request: DispatchRequest): CommandResult {
    ...
    const command = request.command ?? (request.rawCommand ? compileSafeCommand(request.rawCommand, { surface: request.surface ?? "REST", requestedMode: request.requestedMode, correlationId: request.correlationId, idempotencyKey: request.idempotencyKey }).command : null);
    if (!command) {
      const compiled = request.rawCommand ? compileSafeCommand(request.rawCommand, { surface: request.surface ?? "REST", requestedMode: request.requestedMode, correlationId: request.correlationId, idempotencyKey: request.idempotencyKey }) : { diagnostics: [commandDiagnostic("INVALID_ARGUMENT", "Either rawCommand or command is required.")] };
      return { commandId: "uncompiled", executionId: "uncompiled", status: "FAILED", diagnostics: compiled.diagnostics, warnings: [] };
    }
    const resolution = this.resolveCanonical(command);
