...
const safeCommand = (name: string, type: string, args: readonly CommandArgumentSpec[], effectClass: CommandSpec["effectClass"], capabilityKey: string, handlerKey: string, aliases: readonly string[] = [], capabilityState: CapabilityState = "SUPPORTED_WITH_LIMITATIONS"): CommandSpec => schema({
  commandType: type, canonicalName: name, arguments: args, outputSchema: {}, selectionFields: args.filter((argument) => argument.type === "selection").map((argument) => argument.name), objectFields: args.filter((argument) => argument.type === "object").map((argument) => argument.name), stateFields: args.filter((argument) => argument.type === "state").map((argument) => argument.name), settingFields: args.filter((argument) => argument.name === "name" || argument.name === "scope" || argument.name === "targetId").map((argument) => argument.name), effectClass, capabilityKey, capabilityState, safetyClass: "SAFE_TRANSLATABLE", deterministic: true, synchronization: effectClass === "LONG_RUNNING_SCIENTIFIC" ? "AUTO" : "SYNC", resourceClass: effectClass === "LONG_RUNNING_SCIENTIFIC" ? "LONG_RUNNING" : "BOUNDED", handlerKey, provenanceVersion: "r10-provenance.v1", replayVersion: "r10-replay.v1", aliases, pymolReference: { sourceCommit: PYMOL_SOURCE_COMMIT, publicNames: [name], oracleStatus: "ORACLE_PENDING" },
});
