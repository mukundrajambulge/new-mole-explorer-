...
const createGraph = (input: D2AdapterInput, sourceArtifactDigest: ArtifactByteDigest, diagnostics: ReturnType<typeof d2Error>[]): { graph: D2MolecularGraphRevisionV1; identity: D2MolecularIdentityV1; correspondence: D2AtomCorrespondenceMapV1; coordinates: readonly D2CoordinateStateV1[]; chemicalState: D2ChemicalStateV1 } => {
  ...
  const chemicalPayload = { schemaVersion: D2_SCHEMA_VERSION, semanticSchemaId: "D2_CHEMICAL_STATE_V1", molecularIdentityDigest: identity.digest, resolution: "UNKNOWN", protonationStatus: "UNKNOWN", tautomerStatus: "UNKNOWN", formalCharges, stereo: [], selectedComponentIds: componentRows.map((component) => component.componentId), sourceEvidenceRefs: [sourceArtifactDigest] } as const;
  ...
  const coordinateStates = (structure.coordinateStates?.length ? structure.coordinateStates : [undefined]).map((state, index) => {
    ...
    const coordinatePayload = { schemaVersion: D2_SCHEMA_VERSION, semanticSchemaId: "D2_COORDINATE_STATE_V1", chemicalStateDigest: chemicalState.digest, ...(state?.sourceModelNumber !== undefined ? { sourceModelNumber: state.sourceModelNumber } : {}), coordinateFrame: "receptor-source-frame" as D2CoordinateFrameId, coordinateUnits: "ANGSTROM" as const, dimensionality: 3 as const, origin: [f64Bits(0), f64Bits(0), f64Bits(0)] as const, coordinates: coordinateMap, sourceArtifactRefs: [sourceArtifactDigest], stateIndex: index } as const;
