...
type ContextToolbarProps = { activeTool: string; activeCategory: RibbonCategory; collapsed: boolean; representation: RepresentationStyle; colorMode: ColorMode; onAction: (actionId: ActionId) => void; onImport?: () => void; onFetchRcsb: (pdbId: string, mode?: "replace" | "add") => void; onColorMode: (mode: ColorMode) => void; onStyleChange: (style: RepresentationStyle) => void; onToggleCollapsed: () => void; editSelectionCount?: number; editObjectName?: string; editSelectionReady?: boolean; canUndo?: boolean; canRedo?: boolean; onEditBondOrder?: (order: Exclude<BondOrder, "UNKNOWN">) => void };

export const ContextToolbar = ({ activeTool, activeCategory, collapsed, representation, colorMode, onAction, onImport, onFetchRcsb, onColorMode, onStyleChange, onToggleCollapsed, editSelectionCount = 0, editObjectName, editSelectionReady = false, canUndo = false, canRedo = false, onEditBondOrder }: ContextToolbarProps) => {
  ...
  return (
    <section className="context-toolbar" aria-label="Contextual toolbar" data-ribbon-category={activeCategory}>
      ...
      <div className="ribbon-scroll">
        {activeCategory === "Color" ? <div className="ribbon-color-controls" role="group" aria-label="Color controls">
        ...
        </div> : (activeCategory as string) === "Edit" ? <div className="ribbon-edit-controls" role="group" aria-label="Scientific topology editing">
          <div className="ribbon-edit-summary" data-testid="edit-state" data-edit-selection-ready={editSelectionReady ? "true" : "false"}><span className="eyebrow">CANONICAL EDIT</span><strong>{editSelectionCount} atom{editSelectionCount === 1 ? "" : "s"} selected</strong><small>{editObjectName ?? "Select a workspace object"} · {editSelectionReady ? "ready" : editSelectionCount ? "selection unavailable" : "select a canonical target"}</small></div>
          ...
          <button className="tool-button" type="button" onClick={() => onAction("EDIT.ATOM_DELETE")} disabled={!editSelectionReady || editSelectionCount < 1} aria-label="Delete Selected" data-action-id="EDIT.ATOM_DELETE"><Icon name="trash" size={20} /><span>Delete Selected</span></button>
          <button className="tool-button" type="button" onClick={() => onAction("EDIT.BOND_CREATE")} disabled={!editSelectionReady || editSelectionCount !== 2} aria-label="Create Bond" data-action-id="EDIT.BOND_CREATE"><Icon name="plus" size={20} /><span>Create Bond</span></button>
          <button className="tool-button" type="button" onClick={() => onAction("EDIT.BOND_DELETE")} disabled={!editSelectionReady || editSelectionCount !== 2} aria-label="Delete Bond" data-action-id="EDIT.BOND_DELETE"><Icon name="minus" size={20} /><span>Delete Bond</span></button>
          <button className="tool-button" type="button" onClick={() => onAction("EDIT.HYDROGEN_ADD")} disabled={!editSelectionReady || editSelectionCount < 1} aria-label="Add Hydrogens" data-action-id="EDIT.HYDROGEN_ADD"><Icon name="beaker" size={20} /><span>Add Hydrogens</span></button>
          <button className="tool-button" type="button" onClick={() => onAction("EDIT.HYDROGEN_REFILL")} disabled={!editSelectionReady || editSelectionCount < 1} aria-label="Refill Hydrogens" data-action-id="EDIT.HYDROGEN_REFILL"><Icon name="rotate" size={20} /><span>Refill H</span></button>
          <button className="tool-button" type="button" onClick={() => onAction("EDIT.HYDROGEN_REMOVE")} disabled={!editSelectionReady || editSelectionCount < 1} aria-label="Remove Explicit H" data-action-id="EDIT.HYDROGEN_REMOVE"><Icon name="trash" size={20} /><span>Remove Explicit H</span></button>
          <button className="tool-button" type="button" onClick={() => onAction("EDIT.ATOM_ATTACH")} disabled={!editSelectionReady || editSelectionCount !== 1} aria-label="Attach Atom" data-action-id="EDIT.ATOM_ATTACH"><Icon name="atom" size={20} /><span>Attach Atom</span></button>
          <button className="tool-button" type="button" onClick={() => onAction("EDIT.ATOM_REPLACE")} disabled={!editSelectionReady || editSelectionCount !== 1} aria-label="Replace Atom" data-action-id="EDIT.ATOM_REPLACE"><Icon name="shapes" size={20} /><span>Replace Atom</span></button>
          <label className="ribbon-select-label">Bond order<select aria-label="Bond order" disabled={!editSelectionReady || editSelectionCount !== 2 || !onEditBondOrder} defaultValue="SINGLE" onChange={(event) => onEditBondOrder?.(event.target.value as Exclude<BondOrder, "UNKNOWN">)}>{["SINGLE", "DOUBLE", "TRIPLE", "AROMATIC"].map((order) => <option key={order} value={order}>{order}</option>)}</select></label>
        </div> : <>
