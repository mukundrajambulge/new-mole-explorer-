...
const TrajectoryViewer = ({ data }: { data: Extract<BiologicalData, { kind: "TRAJECTORY" }> }) => {
  ...
  return <div className="bio-viewer-content bio-trajectory-viewer" data-testid="trajectory-viewer" data-trajectory-format={data.format} data-trajectory-status={data.status} data-topology-source={data.topology?.sourceName}>
    ...
    <div className="bio-trajectory-summary"><div><span>Frames</span><strong>{data.frames.length ? data.frames.length.toLocaleString() : "Header only"}</strong></div><div><span>Atoms / frame</span><strong>{data.atomCount.toLocaleString()}</strong></div><div><span>Units</span><strong>Å (viewer)</strong></div><div><span>Source</span><strong>{data.format.toUpperCase()}</strong></div>{data.topology && <div><span>Topology</span><strong>{data.topology.sourceName}</strong></div>}</div>
