...
export const ScenePanel = ({ collection, onStore, onRecall, onUpdate, onRename, onDelete, onStep }: ScenePanelProps) => {
  ...
  return <section className="scene-manager" aria-label="Scene manager" data-testid="scene-manager">
    <div className="scene-manager__heading"><div><span className="eyebrow">NATIVE LIFECYCLE</span><h2>Scenes</h2></div><div className="scene-manager__actions"><button type="button" onClick={() => run(() => onStep(-1))} aria-label="Previous scene">Previous</button><button type="button" onClick={() => run(() => onStep(1))} aria-label="Next scene">Next</button></div></div>
