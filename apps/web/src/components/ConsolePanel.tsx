...
export const ConsolePanel = ({ expanded, onToggle, structure }: { expanded: boolean; onToggle: () => void; structure: StructureLoadResult | null }) => {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (!trimmed) return;
    setEntries((current) => [...current, { category: "CAPABILITY", command: trimmed, status: "Not executed · authoritative command service is not connected in G1C", timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" }) }]);
    setQuery("");
  };
