...
const initialEntries: ConsoleEntry[] = [
  ...
  { category: "CAPABILITY", command: "dock run", status: "Unavailable in G1B · no docking engine connected", timestamp: "09:41:15" },
];
...
export const ConsolePanel = ({ expanded, onToggle, structure }: { expanded: boolean; onToggle: () => void; structure: StructureLoadResult | null }) => {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (!trimmed) return;
    setEntries((current) => [...current, { category: "CAPABILITY", command: trimmed, status: "Not executed · authoritative command service is not connected in G1B", timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" }) }]);
    setQuery("");
  };
    ...
    <section className={`console-panel ${expanded ? "console-panel--expanded" : "console-panel--collapsed"}`} aria-label="Command and selection console">
      ...
      {expanded && <>
        <div className="console-history">
          {entries.length === 0 && <div className="console-empty">No command events yet. G1B does not execute scientific queries.</div>}
        ...
        <div className="console-examples"><span className="examples-label">Examples</span><button onClick={() => setQuery("select all")}>select all</button><button onClick={() => setQuery("show capabilities")}>show capabilities</button><button onClick={() => setQuery("dock run")}>dock run</button><button onClick={() => setQuery("help G1B")}>help G1B</button></div>
      </>}
    </section>
  );
};
