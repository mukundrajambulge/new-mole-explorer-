...
type StructurePanelProps = {
  ...
  activeSelection: Pick<SelectionResult, "count" | "status" | "membershipHash" | "query" | "coordinateContext" | "presentationContext" | "stableAtomIds"> | null;
...
const ContextCard = ({ selectedAtom, onAction, onClearSelection }: { selectedAtom: CanonicalAtom | null; onAction: (actionId: ActionId) => void; onClearSelection: () => void }) => (
  <section className="panel-card context-card" data-testid="context-panel">
    <div className="panel-heading"><div><h2>Context</h2></div></div>
...
export type ScientificOperationsPanelProps = { mode: "measure" | "analyze" | "combined"; measurementMode: MeasurementKind | null; measurementSlots: readonly string[]; measurements: readonly MeasurementObject[]; structure: StructureLoadResult | null; onAction: (actionId: ActionId) => void; onMeasurementMode: (kind: MeasurementKind | null) => void; onMeasurementVisibility: (id: string, visible: boolean) => void; onMeasurementDelete: (id: string) => void; onMeasurementClear: () => void; analysisResults: readonly StructuralAnalysisResult[]; fittingResults: readonly (FittingAnalysis & { applyStatus: "ANALYZED" | "APPLIED" | "STALE" })[] };

export const ScientificOperationsPanel = ({ mode, measurementMode, measurementSlots, measurements, structure, onAction, onMeasurementMode, onMeasurementVisibility, onMeasurementDelete, onMeasurementClear, analysisResults, fittingResults }: ScientificOperationsPanelProps) => {
  ...
  return <section className="panel-card analysis-card measurement-card" data-testid="measurements-panel">
    <div className="panel-heading"><div><h2>{mode === "measure" ? "Measure" : mode === "analyze" ? "Analyze" : "Analysis & Interaction"}</h2></div>{mode !== "analyze" && <span className="measurement-count" aria-label={`${measurements.length} measurements`}>{measurements.length}</span>}</div>
      {mode !== "measure" && <><div className="analysis-tool-grid">{analysisTools.map((tool) => <button className="quick-tool" key={tool.label} onClick={() => onAction(tool.actionId)} data-action-id={tool.actionId} title={tool.capability ? `${tool.label} — ${tool.capability}` : tool.label} disabled={tool.capability === "Coming Soon"} aria-disabled={tool.capability === "Coming Soon"}><span className="quick-icon"><Icon name={tool.icon} size={18} /></span><span>{tool.label}</span>{tool.capability && <small>{tool.capability}</small>}</button>)}</div>
      ...
      {fittingResults.length > 0 && <div className="analysis-results alignment-results" data-testid="alignment-results"><div className="analysis-subheading"><span>Alignment results</span></div>{fittingResults.map((entry) => <div className={`analysis-result alignment-result alignment-result--${entry.applyStatus.toLowerCase()}`} key={entry.result.resultId} data-alignment-result-id={entry.result.resultId} data-alignment-disposition={entry.result.resultDisposition}><div><strong>{entry.result.operationKind} · {entry.applyStatus}</strong><span>Current {formatOptional(entry.result.currentRmsd, 4)} Å · fitted {formatOptional(entry.result.evaluationRmsd ?? entry.result.refinedCoreRmsd, 4)} Å · {entry.result.retainedPairCount}/{entry.result.initialPairCount} pairs</span></div><small>{entry.result.resultDisposition} · {entry.result.transformUniqueness}</small></div>)}</div>}</>}
    {mode === "combined" && <div className="analysis-divider" />}
    {mode !== "analyze" && <><div className="analysis-subheading"><span>Measurements</span></div><div className="measurement-toolbar">{measurementKinds.map((kind) => <button type="button" className={`measurement-button ${measurementMode === kind ? "measurement-button--active" : ""}`} key={kind} aria-pressed={measurementMode === kind} onClick={() => { if (!structure) { onAction(`MEASURE.${kind}` as ActionId); return; } onMeasurementMode(measurementMode === kind ? null : kind); }}>{measurementLabel(kind)}</button>)}</div>
...
export const StructurePanel = ({ collapsed, onToggle, onAction, structure, workspaceObjects, workspaceGroups, activeObjectId, coordinateFramePolicy, onCoordinateFrameChange, onObjectSelect, onObjectToggle, onObjectStateCycle, onObjectAllStatesToggle, projection, selectedAtom, activeSelection, onClearSelection, measurementMode, measurementSlots, measurements, onMeasurementMode, onMeasurementVisibility, onMeasurementDelete, onMeasurementClear, analysisResults, fittingResults, onAlignmentCommand, canUndo, canRedo, loading, error, namedSelections, onNamedSelectionAction, showOperations = true, renderReady = true }: StructurePanelProps) => {
  ...
  const selectionOwnerIds = activeSelection
    ? [...new Set(activeSelection.stableAtomIds.map((stableId) => stableId.includes("::") ? stableId.slice(0, stableId.indexOf("::")) : activeObjectId).filter((value): value is string => Boolean(value)))]
    : [];
  const selectionOwners = workspaceObjects.filter((object) => selectionOwnerIds.includes(object.objectId));
  const selectionOwnerLabel = selectionOwners.length ? selectionOwners.map((object) => object.displayName).join(", ") : "None";
  const selectionRelation = selectionOwnerIds.length === 1 && selectionOwnerIds[0] !== activeObjectId ? "Selection owner differs from active object" : selectionOwnerIds.length > 1 ? "Selection spans multiple objects" : "Selection follows active object";
  ...
  return <aside className="side-column side-column--left" aria-label="Structure, context and analysis panel">
    <section className="panel-card structure-card">
      <div className="panel-heading"><div><h2>Structure</h2></div><div className="panel-actions"><button className="icon-button" onClick={onToggle} aria-label="Collapse structure panel"><Icon name="panelLeftClose" size={16} /></button></div></div>
    ...
    <section className="panel-card selections-card" data-testid="objects-selections-panel">
      ...
      {workspaceObjects.map((object) => <div className={`selection-object-row ${object.objectId === activeObjectId ? "selection-object-row--active" : ""}`} key={object.objectId} data-object-id={object.objectId} data-object-name={object.displayName} data-object-enabled={object.enabled ? "true" : "false"}>
        ...
        <button type="button" className="object-enable-button" onClick={() => onObjectToggle(object.objectId)} aria-label={`${object.enabled ? "Disable" : "Enable"} ${object.displayName}`}>{object.enabled ? renderReady ? "ON" : "PREPARING" : "OFF"}</button>
      </div>)}
      {(() => { const activeObject = workspaceObjects.find((object) => object.objectId === activeObjectId); return <div className="active-object-state" data-testid="active-object-state" data-active-object-id={activeObject?.objectId ?? ""} data-active-object-enabled={activeObject?.enabled ? "true" : "false"} data-render-ready={renderReady ? "true" : "false"}><span className="eyebrow">ACTIVE OBJECT</span><strong>{activeObject?.displayName ?? "None"} · {activeObject?.enabled ? renderReady ? "ON" : "PREPARING" : activeObject ? "OFF" : "—"}</strong>{activeObject && <small>{activeObject.objectId} · state {activeObject.currentStateId}</small>}</div>; })()}
      ...
      {activeSelection ? <><div className="active-selection-summary" data-testid="active-selection" data-membership-hash={activeSelection.membershipHash} data-selection-status={activeSelection.status} data-selection-owner-ids={selectionOwnerIds.join(",")} data-selection-owner-names={selectionOwnerLabel} data-active-object-id={activeObjectId ?? ""} data-selection-scope-relation={selectionRelation} data-coordinate-state-scopes={activeSelection.coordinateContext ? JSON.stringify(activeSelection.coordinateContext.stateScopes) : undefined} data-presentation-revision={activeSelection.presentationContext?.revision}><span className="tree-badge tree-badge--cyan">A</span><span className="tree-label">Active selection</span><span className="muted">{formatCount(activeSelection.count)} atoms · {activeSelection.status.replaceAll("_", " ")}</span>{coordinateScopeLabel(activeSelection) && <span className="muted" title={JSON.stringify(activeSelection.coordinateContext?.stateScopes)}>· {coordinateScopeLabel(activeSelection)}</span>}</div><div className="selection-scope" data-testid="selection-scope" data-selection-owner-ids={selectionOwnerIds.join(",")} data-selection-owner-names={selectionOwnerLabel} data-active-object-id={activeObjectId ?? ""} data-selection-scope-relation={selectionRelation}><span className="eyebrow">SELECTION OBJECT</span><strong>{selectionOwnerLabel}</strong><small>{selectionRelation}{activeObjectId ? ` · active object ${workspaceObjects.find((object) => object.objectId === activeObjectId)?.displayName ?? activeObjectId}` : ""}</small></div></> : <div className="selection-empty">No active selection.</div>}
    ...
    <section className="panel-card components-card"><div className="panel-heading"><div><h2>Components</h2></div></div><div className="component-list">{components.map((component) => <div className={`component-row ${component.visible ? "component-row--visible" : "component-row--hidden"}`} key={component.label}><span className={`component-dot component-dot--${component.tone} ${component.visible ? "component-dot--visible" : "component-dot--hidden"}`} aria-hidden="true" /><span>{component.label}</span><span className="component-count">{formatCount(component.count)}</span></div>)}</div></section>
