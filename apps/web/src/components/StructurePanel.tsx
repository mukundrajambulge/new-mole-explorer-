...
export const ScientificOperationsPanel = ({ mode, measurementMode, measurementSlots, measurements, structure, onAction, onMeasurementMode, onMeasurementVisibility, onMeasurementDelete, onMeasurementClear, analysisResults, fittingResults }: ScientificOperationsPanelProps) => {
  ...
  return <section className="panel-card analysis-card measurement-card" data-testid="measurements-panel">
    ...
    {mode !== "measure" && <><div className="analysis-tool-grid">{analysisTools.map((tool) => <button className="quick-tool" key={tool.label} onClick={() => onAction(tool.actionId)} data-action-id={tool.actionId} title={tool.capability ? `${tool.label} — ${tool.capability}` : tool.label}><span className="quick-icon"><Icon name={tool.icon} size={18} /></span><span>{tool.label}</span>{tool.capability && <small>{tool.capability}</small>}</button>)}</div>
